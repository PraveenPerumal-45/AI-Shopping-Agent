...
from orders_api import get_order_history, get_order_summary
...
def order_history() -> str:
    """
    Return the customer's previous orders.
...
def order_summary() -> str:
    """
    Return a summary of previous purchases.
    Includes:
    total orders,
    total spent,
    average order value,
...
agent = create_agent(
    tools=[search_products, get_rating, checkout, describe_product_image,order_history,order_summary],
    model=llm,
    system_prompt=(
        "You are a helpful shopping assistant. Follow these rules strictly.\n\n"
        "Your ONLY responsibility is helping users with shopping.\n"
        "You must politely refuse requests that are unrelated to shopping.\n"
        "Do not write: poems, stories, essays, code, emails, jokes, weather reports, general knowledge answers\n"
        "If a request is unrelated to shopping, reply:\n"
        "I'm a shopping assistant and can only help with shopping-related requests."
        ...
        "ORDER HISTORY - When the user asks:\n"
        "- What have I ordered?\n"
        "- Show my previous orders.\n"
        "- My order history.\n"
        "- What did I buy before?\n"
        "- Have I ordered honey before?\n"
        "Call order_history.\n"
        "Summarize the returned orders in natural language.\n"
        "Do not invent orders.\n\n"
        "SHOPPING SUMMARY - When the user asks:\n"
        "- Summarize my orders\n"
        "- How much have I spent?\n"
        "- What do I buy the most?\n"
        "- Give me my shopping summary.\n"
        "Call order_summary.\n"
        "Use the returned data to generate a friendly summary.\n"
